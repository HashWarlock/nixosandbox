...
pub fn query_catalog() -> Result<String, String> {
  ...
  catalog = flake.catalog;
  filterDrvs = attrs:
    let keys = builtins.filter (k: (attrs.${{k}}.type or "") == "derivation")
                               (builtins.attrNames attrs);
    in builtins.listToAttrs (map (k: {{ name = k; value = attrs.${{k}}; }}) keys);
  extractMeta = attrs:
    builtins.mapAttrs (name: pkg: {{ description = pkg.meta.description or ""; }})
                      (filterDrvs attrs);
in {{ agents = extractMeta catalog.agents; tools = extractMeta catalog.tools; }}"#,
        flake_root
    );
